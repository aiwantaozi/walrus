...
func (l *Loader) loadSchema(
	...
	mod *tfconfig.Module,
	template string,
) (*types.Schema, *types.UISchema, error) {
	// OpenAPISchema.
	...
	openAPISchema := types.OpenAPISchema{
		OpenAPISchema: ts,
	}
...
func (l *Loader) loadData(rootDir string, mod *tfconfig.Module) (
	data types.TemplateData, err error,
) {
	// Readme.
	...
	data.Terraform = types.TerraformMetadata{
