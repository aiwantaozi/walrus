...
import (
	...
	"github.com/seal-io/getter/tlsutil"
...
func (g *HTTPGetter) httpClient() (*http.Client, error) {
	...
	if (len(g.opts.cert) != 0 && len(g.opts.certKey) != 0) || len(g.opts.ca) != 0 || g.opts.insecureSkipVerifyTLS {
		tlsConf, err := tlsutil.NewClientTLS(g.opts.cert, g.opts.certKey, g.opts.ca, g.opts.insecureSkipVerifyTLS)
		if err != nil {
